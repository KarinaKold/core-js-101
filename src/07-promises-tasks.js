...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolution, rejection) => {
    if (typeof isPositiveAnswer !== 'boolean') {
      rejection(new Error('Wrong parameter is passed! Ask her again.'));
    }
    if (isPositiveAnswer === true) {
      resolution('Hooray!!! She said "Yes"!');
    }
    if (isPositiveAnswer === false) {
      resolution('Oh no, she said "No".');
    }
  });
}
...
function chainPromises(array, action) {
  const promise = [];
  return array.map((val) => Promise.resolve(val)
    .then((p) => {
      promise.push(p);
    })
    .then(() => promise.reduce(action))
    .catch(() => {}))[0];
}
